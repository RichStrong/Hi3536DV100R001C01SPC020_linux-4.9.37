...
static s32 crypto_mem_alloc_remap(crypto_mem *mem, u32 type, char const *name, u32 size)
{
    ...
    ret = CMPI_MmzMallocNocache(NULL, (char*)name, &ADDR_U64(mem->mmz_addr), (void**)&mem->dma_virt, size);
    if (HI_SUCCESS != ret)
    {
...
{
    u32 i;
    CMPI_MmzFree(ADDR_U64(mem->mmz_addr), mem->dma_virt);
...
static s32 crypto_mem_map(crypto_mem *mem)
{
    ...
    mem->dma_virt = (HI_U8*)CMPI_Remap_Nocache(ADDR_U64(mem->dma_addr), mem->dma_size);
