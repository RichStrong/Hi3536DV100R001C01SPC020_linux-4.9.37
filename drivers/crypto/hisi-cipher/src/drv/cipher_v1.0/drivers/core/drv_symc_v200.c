...
static u32 drv_symc_done_try(u32 chn_num)
{
    ...
    return (u32)(status.bits.cipher_chn_obuf_raw ? HI_TRUE : HI_FALSE);
}
...
s32 drv_symc_init(void)
{
    ...
    memset_s(&symc_dma, sizeof(symc_dma), 0, sizeof(symc_dma));
    memset_s(&hard_context, sizeof(hard_context), 0, sizeof(hard_context));
...
s32 drv_symc_set_iv(u32 chn_num, u32 iv[SYMC_IV_MAX_SIZE_IN_WORD], u32 ivlen, u32 flag)
{
    u32 i;
    symc_hard_context *ctx = HI_NULL;
...
s32 drv_aead_ccm_add_n(u32 chn_num, u8 *n)
{
    ...
    memcpy_s(via_pad, SYMC_CCM_N_LEN, n, SYMC_CCM_N_LEN);
...
s32 drv_aead_ccm_add_a(u32 chn_num, compat_addr buf_phy, u32 buf_size)
{
    ...
    ADDR_U64(dma_pad) = ADDR_U64(ctx->dma_pad) + ctx->offset_pad;
    via_pad = ctx->via_pad + ctx->offset_pad;
    memset_s(via_pad, AES_BLOCK_SIZE * 2, 0, AES_BLOCK_SIZE * 2);
...
s32 drv_aead_gcm_add_a(u32 chn_num, compat_addr buf_phy, u32 buf_size)
{
    ...
    ADDR_U64(dma_pad) = ADDR_U64(ctx->dma_pad) + ctx->offset_pad;
    via_pad = ctx->via_pad + ctx->offset_pad;
    memset_s(via_pad, AES_BLOCK_SIZE, 0, AES_BLOCK_SIZE);
...
s32 drv_aead_gcm_add_clen(u32 chn_num, u8 *clen)
{
    ...
    memcpy_s(via_pad, SYMC_GCM_CLEN_LEN, clen, SYMC_GCM_CLEN_LEN);
...
s32 drv_aead_get_tag(u32 chn_num, u32 *tag)
{
    ...
    memcpy_s(tag, AEAD_TAG_SIZE_IN_WORD * 4, ctx->entry_out[last].tag,
...
s32 drv_symc_set_destory_callbcak(u32 chn_num, callback_symc_destory destory)
{
    symc_hard_context *ctx = HI_NULL;
...
void drv_symc_get_capacity(symc_capacity *capacity)
{
    ...
    */
    ...
    memset_s(capacity, sizeof(symc_capacity), 0,  sizeof(symc_capacity));
