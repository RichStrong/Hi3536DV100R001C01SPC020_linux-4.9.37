...
s32 module_addr_map(void)
{
    ...
    /* use non-secure info */
    memcpy_s(&hard_info_table, sizeof(hard_info_table),
...
static void module_reset(module_id id, u32 enable)
{
    ...
    if (table->reset_valid)
    {
        ...
        {
            val = crypto_read((u8 *)table->reg_addr_via + table->version_reg);
            if (val != expect)
            {
...
s32 crypto_channel_init(channel_context *ctx, u32 num, u32 ctx_size)
{
    ...
    if (ctx_size > 0)
    {
        ...
        memset_s(buf, ctx_size * num, 0, ctx_size * num);
