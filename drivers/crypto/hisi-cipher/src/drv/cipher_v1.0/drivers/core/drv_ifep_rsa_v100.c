...
static s32 drv_rsa_set_key(u32 catype, u8 *n, u8 *d, u32 klen, u32 random[2])
{
    u32 i =0, id = 0;
    u32 val = 0x00;
    s32 ret = HI_FAILURE;
    ...
    {
        memcpy_s(&val, sizeof(u32), n + i, WORD_WIDTH);
        val ^= random[id];
    ...
    /* Set D */
    if (HI_UNF_CIPHER_KEY_SRC_USER !=  catype )
    {
        ret = DRV_Cipher_KladLoadKey(0, catype, HI_UNF_CIPHER_KLAD_TARGET_RSA, d, klen);
        if( HI_SUCCESS != ret )
        {
            HI_LOG_ERROR("DRV_Cipher_KladLoadKey, error!\n");
            return HI_FAILURE;
        }
    }
    else
    {
        for (i=0; i<klen; i+=WORD_WIDTH)
        {
            memcpy_s(&val, sizeof(u32), d + i, WORD_WIDTH);
            val ^= random[id];
            IFEP_RSA_WRITE(REG_SEC_RSA_WSEC_REG, val);
...
static void drv_rsa_set_input(u8 *in, u32 klen)
{
    ...
    for (i=0; i<klen; i+=WORD_WIDTH)
    {
        memcpy_s(&val, sizeof(u32), in + i, WORD_WIDTH);
...
static void drv_rsa_get_output(u8 *out, u32 klen)
{
    ...
    for (i=0; i<klen; i+=4)
    {
        ...
        memcpy_s(out + i, sizeof(u32), &val, 4);
    }
    return;
}
...
s32 drv_ifep_rsa_exp_mod(u32 catype, u8 *n, u8 *k, u8 *in, u8 *out, rsa_key_width width)
{
    s32 ret = HI_FAILURE;
    u32 klen = 0;
    u32 random[2] = {0, 0};
    ...
    /* set rsa key */
    ret = drv_rsa_set_key(catype, n, k, klen, random);
    if (HI_SUCCESS != ret)
    {
        HI_LOG_PrintFuncErr(drv_rsa_set_key, ret);
...
{
    memset_s(capacity, sizeof(rsa_capacity), 0,  sizeof(rsa_capacity));
