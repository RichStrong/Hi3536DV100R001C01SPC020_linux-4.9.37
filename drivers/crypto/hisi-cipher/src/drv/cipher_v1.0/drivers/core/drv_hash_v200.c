...
s32 drv_hash_init(void)
{
    ...
    if (HI_SUCCESS != ret)
    {
        ...
        ret_error = crypto_mem_destory(&hash_dma);
        if (HI_SUCCESS != ret_error)
        {
            HI_LOG_ERROR("error, hash dma crypto mem destory failed.\n");
            HI_LOG_PrintFuncErr(crypto_mem_destory, ret_error);
        }
        goto __error1;
    }
#endif
...
__error1:
    ...
    if (HI_SUCCESS != ret_error)
    {
        ...
        HI_LOG_PrintFuncErr(crypto_channel_deinit, ret_error);
    }
...
s32 drv_hash_deinit(void)
{
    ...
    ret = crypto_mem_destory(&hash_dma);
    if (HI_SUCCESS != ret)
    {
