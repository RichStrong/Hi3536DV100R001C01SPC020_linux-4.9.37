...
#define RSA_PKCS1_TYPE_MIN_PAD_LEN               (0x11)
...
static u32 rsa_key_catype = HI_UNF_CIPHER_KEY_SRC_USER;
...
static int cryp_ifep_rsa_exp_mod(u32 catype, mbedtls_mpi *X, const mbedtls_mpi *A,
                            const mbedtls_mpi *E, const mbedtls_mpi *N,
                            mbedtls_mpi *_RR)
{
    u32 module_len = 0;
    ...
    /* zero */
    memset_s(buf, keylen * 4, 0, keylen * 4);
    ...
    /* out = in ^ k mod n */
    ret = drv_ifep_rsa_exp_mod(catype, n, k, in, out, width);
    if (HI_SUCCESS == ret)
    {
        /* write d */
                         ...
                         mbedtls_mpi *_RR)
{
     ...
     */
    ret = cryp_ifep_rsa_exp_mod(rsa_key_catype, X, A, E, N, _RR);
#else
    if (HI_UNF_CIPHER_KEY_SRC_USER != rsa_key_catype)
    {
    ...
    *(peb++) = bt;
    ...
    switch(bt)
    {
        case 0x00:
        {
            ...
            memset_s(peb, plen, 0x00, plen);
            break;
        }
        case 0x01:
        {
            /* for block type 01, they shall have value FF */
            memset_s(peb, plen, 0xFF, plen);
            break;
        }
        case 0x02:
        {
                                       ...
                                       u8 *in, u32 inlen, u8 *out)
{
    ...
    memset_s(data, RSA_MAX_KEY_LEN, 0, RSA_MAX_KEY_LEN);
    memcpy_s(data + klen - inlen, RSA_MAX_KEY_LEN, in, inlen);
