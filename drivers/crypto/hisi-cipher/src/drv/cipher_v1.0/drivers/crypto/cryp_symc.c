...
s32 cryp_symc_init(void)
{
    ...
    memset_s(&symc_descriptor, sizeof(symc_descriptor), 0, sizeof(symc_descriptor));
...
static void * cryp_symc_create(u32 hard_chn)
{
    ...
    memset_s(hisi_ctx, sizeof(cryp_symc_context), 0, sizeof(cryp_symc_context));
...
static void cryp_symc_setkey(void *ctx, const u8 *key, u32 keylen, u32 odd)
{
    ...
    if (odd)
    {
        memcpy_s(hisi_ctx->odd_key, SYMC_KEY_SIZE, key, keylen);
    }
    else
    {
        memcpy_s(hisi_ctx->even_key, SYMC_KEY_SIZE, key, keylen);
    }
...
static void cryp_symc_setiv(void *ctx, const u8 *iv, u32 ivlen, u32 usage)
{
    ...
    memcpy_s(hisi_ctx->iv, AES_IV_SIZE, iv, ivlen);
...
static void cryp_symc_getiv(void *ctx, u8 *iv, u32 *ivlen)
{
    ...
    memcpy_s(iv, AES_IV_SIZE, hisi_ctx->iv, hisi_ctx->ivlen);
...
static s32 cryp_symc_sm1_setsk(void *ctx, const u8 *key)
{
    ...
    memcpy_s(hisi_ctx->sk, SYMC_SM1_SK_SIZE, key, SYMC_SM1_SK_SIZE);
...
static s32 cryp_symc_config(void *ctx, u32 decrypt)
{
    cryp_symc_context *hisi_ctx = ctx;
    symc_klen type;
    s32 ret = HI_FAILURE;
    ...
    if (HI_SUCCESS != ret)
    {
        HI_LOG_PrintFuncErr(drv_symc_set_iv, ret);
        return ret;
    }
                    ...
                    u32 pkg_num, symc_node_usage usage)
{
    ...
    if (HI_SUCCESS != ret)
    {
        HI_LOG_PrintFuncErr(cryp_symc_config, ret);
        return ret;
    }
...
static s32 cryp_aead_ccm_setiv(void *ctx, const u8 *iv, u32 ivlen, u32 usage)
{
     ...
     */
    memset_s(ccm_iv, sizeof(ccm_iv), 0, AES_IV_SIZE);
    ccm_iv[0] = AES_CCM_NQ_LEN - ivlen; /*IV[0] = q - 1 = 15 - n -1*/
    memcpy_s(&ccm_iv[1], sizeof(ccm_iv)-1, iv, ivlen);
    ivlen += 1;
                          ...
                          u32 alen, u32 enclen, u32 tlen)
{
     ...
     *  ---------------------------------------------
    */
    memset_s(buf, AES_BLOCK_SIZE, 0, AES_BLOCK_SIZE);
    ...
    memcpy_s(&buf[idx], AES_BLOCK_SIZE - idx, &iv[1], ivlen - 1);
    idx += ivlen - 1;
...
static s32 cryp_aead_get_tag(void *ctx, u32 tag[AEAD_TAG_SIZE_IN_WORD], u32 *taglen)
{
    ...
    *taglen = hisi_ctx->tlen;
    ...
    memcpy_s(tag, AEAD_TAG_SIZE, hisi_ctx->tag, hisi_ctx->tlen);
...
static s32 cryp_register_symc(symc_func *func)
{
    ...
    for(i=0; i<SYMC_FUNC_TAB_SIZE; i++)
    {
        if (!symc_descriptor[i].valid)
        {
            memcpy_s(&symc_descriptor[i], sizeof(symc_func), func, sizeof(symc_func));
...
static s32 cryp_sm1_setkey(void *ctx, const u8 *fkey, const u8 *skey, u32 *hisi_klen)
{
    ...
    if(HI_SUCCESS != ret)
    {
        HI_LOG_PrintFuncErr(cryp_symc_sm1_setsk, ret);
        return ret;
    }
...
{
    memset_s(func, sizeof(symc_func), 0, sizeof(symc_func));
...
static void cryp_register_symc_dma(u32 dma_capacity, u32 tdes_capacity)
{
    ...
    memset_s(&func, sizeof(func), 0, sizeof(func));
...
static void cryp_register_all_symc(void)
{
    ...
    memset_s(&capacity, sizeof(capacity), 0, sizeof(capacity));
