...
s32 kapi_symc_init(void)
{
    ...
    memset_s(&kapi_ctx, sizeof(kapi_ctx), 0, sizeof(kapi_ctx));
...
s32 kapi_symc_create(u32 *id)
{
    ...
    memset_s(ctx, sizeof(kapi_symc_ctx), 0, sizeof(kapi_symc_ctx));
    ...
    */
    ...
    if (ctx->func->setkey)
    {
        if (HI_TRUE == byca)
        {
            ...
            CHECK_EXIT(klad_load_hard_key(id, catype, fkey));
        }
        else
        {
    ...
    /* save crtl */
    memset_s(&ctx->ctrl, sizeof(CRYP_CIPHER_CTRL_S), 0, sizeof(CRYP_CIPHER_CTRL_S));
    ...
    if (HI_NULL != iv)
    {
        ...
        memcpy_s(ctx->ctrl.u32IV, AES_IV_SIZE, iv, ivlen);
    }
    if (HI_NULL != fkey)
    {
        ...
        memcpy_s(ctx->ctrl.u32Key, AES_KEY_256BIT, fkey, klen);
    }
...
s32 kapi_symc_get_config(u32 id, CRYP_CIPHER_CTRL_S *ctrl)
{
    ...
    ctx = &kapi_ctx[softId];
    CHECK_OWNER(&ctx->owner);
    HI_LOG_CHECK_PARAM(HI_FALSE == ctx->config);

    KAPI_SYMC_LOCK();

    memcpy_s(ctrl, sizeof(CRYP_CIPHER_CTRL_S), &ctx->ctrl, sizeof(CRYP_CIPHER_CTRL_S));
