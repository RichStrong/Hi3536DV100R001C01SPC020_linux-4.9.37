...
/* Set key */
int sm4_set_encrypt_key(const u8 *userKey, const int bits, SM4_KEY_S *key)
{
    int i = 0;
    u32 k_temp[4] = {0};
    u32 K[36] = {0};
...
void sm4_cbc_crypt(const u8 *in, u8 *out, const u32 length, const SM4_KEY_S *key, u8 *ivec, const int dec)
{
    ...
    if (SYMC_OPERATION_ENCRYPT == dec)
    {
        ...
        if (len)
        {
            ...
            sm4_encrypt(tmp, out, key);
            memcpy(out, tmp, len);
            memcpy((void *)ivec, tmp, sizeof tmp);
        }
    }
    else
    {
...
/* SM4 ECB RM */
u32 sm4_ecb_rm(const u8 *data_in, u8 *data_out, int data_len, const u8 *key, int bit, u32 decrypt)
{
    SM4_KEY_S ctx;
...
/* SM4 CBC RM */
u32 sm4_cbc_rm(const u8 *data_in, u8 *data_out, int data_len,
...
/* SM4 CTR RM */
u32 sm4_ctr_rm(const u8 *data_in, u8 *data_out, int data_len,
                        const u8 *key, int bit, u32 decrypt,
                        const u8 *iv)
{
    int num = 0;
...
void * ext_sm4_create(u32 hard_chn)
{
    ...
    memset_s(ctx, sizeof(ext_sm4_context), 0, sizeof(ext_sm4_context));
...
s32 ext_sm4_setiv(void *ctx, const u8 *iv, u32 ivlen, u32 usage)
{
    ...
    memcpy_s(symc->iv, AES_IV_SIZE, iv, ivlen);
...
void ext_sm4_getiv(void *ctx, u8 *iv, u32 *ivlen)
{
    ...
    memcpy_s(iv, AES_IV_SIZE, symc->iv, AES_IV_SIZE);
    *ivlen = AES_IV_SIZE;
...
s32 ext_sm4_setkey(void *ctx, const u8 *fkey, const u8 *skey, u32 *hisi_klen)
{
    ...
    memcpy_s(symc->key, SYMC_KEY_SIZE, fkey, klen);
    symc->klen = klen;
    *hisi_klen = klen;
