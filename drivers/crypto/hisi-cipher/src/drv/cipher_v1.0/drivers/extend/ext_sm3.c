...
void sm3_compress(u32 digest[SM3_RESULT_SIZE_IN_WORD], const u8 block[SM3_BLOCK_SIZE])
{
    s32 j;
    u32 W[68] = {0}, W1[64] = {0};
...
static void sm3_update(ext_sm3_context *ctx, const u8* data, u32 data_len)
{
    u32 left = 0;
    ...
    if (ctx->tail_len)
    {
        ...
        if (data_len < left)
        {
            memcpy(ctx->tail + ctx->tail_len, data, data_len);
            ctx->tail_len += data_len;
            return;
        }
        else
        {
            memcpy(ctx->tail + ctx->tail_len, data, left);
            sm3_compress(ctx->state, ctx->tail);
...
static void sm3_final(ext_sm3_context *ctx, u8 *digest)
{
    s32 i = 0;
    ...
    {
        memset(ctx->tail + ctx->tail_len + 1, 0,
            SM3_BLOCK_SIZE - ctx->tail_len - 1);
    }
    else
    {
        /* 2 block is request */
        memset(ctx->tail + ctx->tail_len + 1, 0, SM3_BLOCK_SIZE - ctx->tail_len - 1);
        sm3_compress(ctx->state, ctx->tail);
        memset(ctx->tail, 0, SM3_BLOCK_SIZE);
    }
    ...
    memcpy_s(digest, SM3_RESULT_SIZE, hash, SM3_RESULT_SIZE);
...
void * ext_sm3_create(hash_mode mode)
{
    ...
    memset_s(ctx, sizeof(ext_sm3_context), 0, sizeof(ext_sm3_context));
...
s32 ext_sm3_update(void *ctx, u8 *chunk, u32 chunkLen, hash_chunk_src src)
{
    ...
    if (HASH_CHUNCK_SRC_USER == src)
    {
        ...
        memset_s(ptr, chunkLen, 0, chunkLen);
        ret = crypto_copy_from_user(ptr, chunk, chunkLen);
        if (HI_SUCCESS != ret)
        {
        ...
        sm3_update(ctx, ptr, chunkLen);
        ret = HI_SUCCESS;
    }
    else
    {
        sm3_update(ctx, chunk, chunkLen);
        ret = HI_SUCCESS;
    }
