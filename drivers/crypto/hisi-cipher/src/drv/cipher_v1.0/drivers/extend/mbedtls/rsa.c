...
/* RSA config f_rng as NULL, if non-null point define this, use non-null f_rng */
//#define FRNG_NON_NULLPOINT
...
static void mbedtls_get_hash_info(mbedtls_md_type_t type,
                                  unsigned int *hlen,
                                  CRYP_CIPHER_HASH_TYPE_E *hisi_type)
{
    CRYP_CIPHER_HASH_TYPE_E shatype = CRYP_CIPHER_HASH_TYPE_SHA1;

    switch(type)
    {
        case MBEDTLS_MD_SHA1:
            *hlen = 20;
            shatype = CRYP_CIPHER_HASH_TYPE_SHA1;
            break;
        case MBEDTLS_MD_SHA224:
            *hlen = 28;
            shatype = CRYP_CIPHER_HASH_TYPE_SHA224;
            break;
        case MBEDTLS_MD_SHA256:
            *hlen = 32;
            shatype = CRYP_CIPHER_HASH_TYPE_SHA256;
            break;
        case MBEDTLS_MD_SHA384:
            *hlen = 48;
            shatype = CRYP_CIPHER_HASH_TYPE_SHA384;
            break;
        case MBEDTLS_MD_SHA512:
            *hlen = 64;
            shatype = CRYP_CIPHER_HASH_TYPE_SHA512;
            break;
        default:
            *hlen = 0;
            shatype = 0;
                      ...
                      size_t slen, mbedtls_md_type_t type )
{
    ...
    size_t i, use_len;
    CRYP_CIPHER_HASH_TYPE_E hisi_type = 0;
    u32 hashid = 0;
                            ...
                            unsigned char *output )
{
    ...
    *p++ = 0;
    ...
    /* Construct DB */
    MBEDTLS_MPI_CHK(kapi_hash_start( &hashid, hisi_type, NULL, 0 ));
    MBEDTLS_MPI_CHK(kapi_hash_update( hashid, (u8*)label, label_len, HASH_CHUNCK_SRC_LOCAL));
                         ...
                         unsigned char *sig )
{
    ...
    *p++ = 0x01;
    ...
    MBEDTLS_MPI_CHK(kapi_hash_update( hashid, (u8*)hash, hashlen, HASH_CHUNCK_SRC_LOCAL ));
    ...
    /* maskedDB: Apply dbMask to DB */
    mgf_mask( sig + offset, olen - hlen - 1 - offset, p, hlen, ctx->hash_id );
...
//    int ret;
    ...
    if( md_alg != MBEDTLS_MD_NONE )
    {
        ...
        mbedtls_get_hash_info(md_alg, &hashlen, HI_NULL);
    }
    nb_pad -= hashlen;
                               ...
                               const unsigned char *sig )
{
    ...
    mgf_mask( p, siglen - hlen - 1, p + siglen - hlen - 1, hlen, ctx->hash_id );
    ...
    /* Generate H = Hash( M' ) */
    MBEDTLS_MPI_CHK(kapi_hash_start( &hashid, hisi_type, NULL, 0 ));
    MBEDTLS_MPI_CHK(kapi_hash_update( hashid, zeros, 8, HASH_CHUNCK_SRC_LOCAL ));
    MBEDTLS_MPI_CHK(kapi_hash_update( hashid, (u8*)hash, hashlen, HASH_CHUNCK_SRC_LOCAL ));
    MBEDTLS_MPI_CHK(kapi_hash_update( hashid, p, slen, HASH_CHUNCK_SRC_LOCAL ));
    MBEDTLS_MPI_CHK(kapi_hash_finish( hashid, result, &hisi_hlen ));
