...
void * ext_mbedtls_symc_create(u32 hard_chn)
{
    ...
    memset_s(ctx, sizeof(ext_symc_context), 0, sizeof(ext_symc_context));
...
s32 ext_mbedtls_symc_getiv(void *ctx, u8 *iv, u32 *ivlen)
{
    ext_symc_context *symc = ctx;
    ...
    HI_LOG_CHECK_PARAM(HI_NULL == symc);
    HI_LOG_CHECK_PARAM(HI_NULL == iv);
    HI_LOG_CHECK_PARAM(HI_NULL == ivlen);

    memcpy_s(iv, AES_IV_SIZE, symc->cipher.iv, symc->cipher.iv_size);
