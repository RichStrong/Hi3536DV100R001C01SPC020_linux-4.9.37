...
static int hisi_fmc_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_u32(dev->of_node, "max-dma-size", &dma_size)) {
		dev_err(dev, "Please set the max dma size: 0x%x\n", dma_size);
		return -ENOMEM;
	}
	...
	fmc->buffer = dmam_alloc_coherent(dev, dma_size,
...
static int hisi_fmc_remove(struct platform_device *pdev)
{
	...
	dmam_free_coherent(&pdev->dev, dma_size,
